...
func main() {
	...
	if _, err := os.Stat(dir); os.IsNotExist(err) {
		log.Panicf("GSS error: directory %q not found ❌\n", dir)
	}
...
func startServer() {
	...
	if err != nil {
		log.Panicf("GSS error: the server crashed: %v ❌\n", err)
	}
}
...
func setUpYAML() {
	...
	if err != nil {
		log.Panicf("GSS error: the YAML file could not be read: %v ❌\n", err)
	...
	if err != nil {
		log.Panicf("GSS error: the YAML file content could not be processed: %v ❌\n", err)
